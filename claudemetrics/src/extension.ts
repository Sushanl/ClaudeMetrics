                    ...
                    toolUses.forEach((tool: any) => {
                        ...
                        if (tool.name === 'Edit') {
                            ...
                            if (tool.input.old_string && tool.input.new_string) {
                                ...
                                // Cursor-style counting: for replacements count only added lines, 
                                // for pure deletions count deleted lines
                                if (addedLines > 0 && removedLines > 0) {
                                    // Replacement: count only added lines
                                    linesInThisTool = addedLines;
                                } else {
                                    // Pure addition or pure deletion: count the actual lines
                                    linesInThisTool = addedLines + removedLines;
                                }
                            ...
                            if (tool.input.edits) {
                                tool.input.edits.forEach((edit: any) => {
                                    if (edit.old_string && edit.new_string) {
                                        ...
                                        // Cursor-style counting: for replacements count only added lines,
                                        // for pure deletions count deleted lines
                                        let editChangedLines: number;
                                        if (addedLines > 0 && removedLines > 0) {
                                            // Replacement: count only added lines
                                            editChangedLines = addedLines;
                                        } else {
                                            // Pure addition or pure deletion: count the actual lines
                                            editChangedLines = addedLines + removedLines;
                                        }
